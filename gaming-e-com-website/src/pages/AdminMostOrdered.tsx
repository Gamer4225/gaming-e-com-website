...
function AdminMostOrdered({ setCurrentPage }: Props) {
  const { user, token } = useAuth();
  const [data, setData] = useState<any[]>([]);
  const [sort, setSort] = useState<"asc"|"desc">("desc");
  const load = useCallback(() => { if (!token) return; adminFetch("/api/admin/most-ordered", token).then(r => r.json()).then(setData); }, [token]);
  useEffect(() => { load(); }, [load]);
  ...
  return (
    <div className="admin-body">
      <div className="toolbar"><div className="select-box"><select value={sort} onChange={e => setSort(e.target.value as "asc"|"desc")}><option value="desc">Most Ordered ↓</option><option value="asc">Least Ordered ↑</option></select></div></div>
      <div className="card"><div className="tbl-wrap"><table className="tbl">
        <thead><tr><th>#</th><th>Product</th><th>Brand</th><th>Category</th><th>Price</th><th>Times Ordered</th><th>Units Sold</th><th>Stock</th></tr></thead>
        <tbody>{sorted.map((p,i) => (
          <tr key={p.id} style={p.stock===0?{opacity:.45}:{}}>
            ...
            <td><span className="badge badge-blue">{p.timesOrdered}</span></td>
            <td><span className="badge badge-green">{p.orderCount}</span></td>
